...
shared_examples 'for an abstract layer' do
  let(:network) { double('neural network') }
  
  describe 'registers with the LayerFactory' do
    it { expect(Neural::LayerFactory.find_type(described_class.to_s)).to be(described_class) }

    context 'with a mock network' do
      it { expect(Neural::LayerFactory.from_hash(subject.to_hash(network), network)).to eq(subject) }
    end
  end
  ...
  if described_class.instance_methods.include?("activation_function")
    Neural.debug("#{described_class} has an activation function.")
