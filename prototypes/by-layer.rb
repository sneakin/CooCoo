...
def option_parser options
  @opts = CooCoo::OptionParser.new do |o|
    ...
    o.on('--convolution-box WIDTH,HEIGHT') do |n|
      ...
      h ||= w
      options.layers << [ :conv_box, [w, h], options.conv_size, options.conv_step, options.conv_hidden_out, options.activation_function, (options.conv_updater || :sum).to_sym ]
    end
...
def generate(options, input_size, output_size)
  ...
  options.layers.each_with_index do |(kind, size, *args), i|
    ...
    when :fully_connected
      ...
      last_size = [ size ]
    when :linear then
      net.layer(CooCoo::LinearLayer.new(last_size.reduce(1, &:*), args[0]))
      last_size = [ size ]
    when :recurrent_frontend then
      recurrent = CooCoo::Recurrence::Frontend.new(last_size.reduce(1, &:*), size)
      net.layer(recurrent)
    ...
    when :conv_box then
      csize, cstep, cout, af, updater = args
      ...
      layer = CooCoo::Convolution::BoxLayer.new(*size, *cstep, int_layer, *csize, *cout, updater)
      net.layer(layer)
      last_size = [ layer.output_width, layer.output_height ]
    end
  end

  if last_size.reduce(1, &:*) != output_size
    log.puts("\tLayer #{net.layers.size}\t\t#{last_size}\t#{output_size}")
    net.layer(CooCoo::Layer.new(last_size.reduce(1, &:*), output_size, options.activation_function))
  end

  if options.softmax
    log.puts("\tSoftmax\t#{output_size}")
    net.layer(CooCoo::LinearLayer.new(output_size, CooCoo::ActivationFunctions::ShiftedSoftMax.instance))
  end
